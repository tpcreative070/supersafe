 ...
 */
package co.tpcreative.supersafe.common.hiddencameraimport
...
class CameraPreview(context: Context, private val mCameraCallbacks: CameraCallbacks?) : SurfaceView(context), SurfaceHolder.Callback {
    ...
    override fun surfaceChanged(surfaceHolder: SurfaceHolder?, i: Int, i1: Int, i2: Int) {
        ...
        cameraSize = when (mCameraConfig.getResolution()) {
            CameraResolution.Companion.HIGH_RESOLUTION -> pictureSizes[0] //Highest res
            CameraResolution.Companion.MEDIUM_RESOLUTION -> pictureSizes[pictureSizes.size / 2] //Resolution at the middle
            CameraResolution.Companion.LOW_RESOLUTION -> pictureSizes[pictureSizes.size - 1] //Lowest res
    ...
    fun takePictureInternal() {
        ...
        if (mCamera != null) {
            mCamera.takePicture(null, null, object : PictureCallback {
                override fun onPictureTaken(bytes: ByteArray?, camera: Camera?) {
                    Thread(Runnable { //Convert byte array to bitmap
                        ...
                        if (HiddenCameraUtils.saveImageFromFile(rotatedBitmap, mCameraConfig.getImageFile(),
                                        mCameraConfig.getImageFormat())) {
                            //Post image file to the main thread
                            Handler(Looper.getMainLooper()).post(Runnable { mCameraCallbacks.onImageCapture(mCameraConfig.getImageFile(), mCameraConfig.getPin()) })
                        } else {
