package co.tpcreative.supersafe.ui.sharefilesimport
...
class ShareFilesActivity : BaseActivityNone() {
    ...
    fun onHandlerIntent() {
        try {
            ...
            Utils.Companion.Log(TAG, "original type :$type")
            if (Intent.ACTION_SEND == action && type != null) {
                handleSendSingleItem(intent)
            } else if (Intent.ACTION_SEND_MULTIPLE == action && type != null) {
                handleSendMultipleFiles(intent, EnumFormatType.FILES)
            } else {
                Utils.Companion.Log(TAG, "Sending items is not existing")
            }
        } catch (e: Exception) {
            finish()
            e.printStackTrace()
    ...
    fun handleSendSingleItem(intent: Intent?) {
        try {
            ...
            val mainCategories: MainCategoryModel? = list[0]
            if (imageUri != null && mainCategories != null) {
                mListFile.clear()
                onStartProgressing()
                var response = ""
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                    Utils.Companion.Log(TAG, "value path :" + imageUri.path)
                    Utils.Companion.Log(TAG, "Path existing " + storage.isFileExist(imageUri.path))
                ...
                if (response == null) {
                    onStopProgressing()
                    Utils.Companion.showGotItSnackbar(imgChecked, R.string.error_occurred, object : ServiceManagerSyncDataListener {
                    ...
                    if (mFile.exists()) {
                        ...
                        Utils.Companion.Log(TAG, "file extension $fileExtension")
                        Utils.Companion.Log(TAG, "Path file :$path")
                        var mimeTypeFile: MimeTypeFile = Utils.Companion.mediaTypeSupport().get(fileExtension)
                        if (mimeTypeFile == null) {
                            val mMimeTypeSupport: MimeTypeFile = Utils.Companion.mimeTypeSupport().get(mimeType)
                            if (mMimeTypeSupport != null) {
                                when (mMimeTypeSupport.formatType) {
                                    EnumFormatType.IMAGE -> {
                                        ...
                                        mimeTypeFile.name = Utils.Companion.getCurrentDateTime(Utils.Companion.FORMAT_TIME_FILE_NAME) + mMimeTypeSupport.extension
                                    }
                                ...
                                Utils.Companion.Log(TAG, "type file $mimeType")
                            }
                        }
                        ...
                        ServiceManager.Companion.getInstance().setListImport(mListImport)
                ...
                Utils.Companion.Log(TAG, "Nothing to do at single item")
            }
        } catch (e: Exception) {
            e.printStackTrace()
        }
    ...
    fun handleSendMultipleFiles(intent: Intent?, enumFormatType: EnumFormatType?) {
        try {
            ...
            if (imageUris != null) {
                ...
                for (i in imageUris.indices) {
                    ...
                    if (response != null) {
                        ...
                        if (mFile.exists()) {
                            ...
                            mListImport.add(importFiles)
                            Utils.Companion.Log(TAG, Gson().toJson(mimeTypeFile))
                        } else {
                            onStopProgressing()
                            finish()
                        }
                ...
                finish()
                Utils.Companion.Log(TAG, "Nothing to do at multiple items")
            }
        } catch (e: Exception) {
            finish()
            e.printStackTrace()
    ...
    protected override fun onResume() {
        ...
        ServiceManager.Companion.getInstance().setRequestShareIntent(true)
        Utils.Companion.onScanFile(this, "scan.log")
    }
    ...
    private fun onStopProgressing() {
        Utils.Companion.Log(TAG, "onStopProgressing")
        try {
            runOnUiThread(Runnable {
                if (dialog != null) {
                    dialog.dismiss()
