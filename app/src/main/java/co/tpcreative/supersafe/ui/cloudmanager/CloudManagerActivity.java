...
import co.tpcreative.supersafe.model.DriveType;
...
public class CloudManagerActivity extends BaseGoogleApi implements UploadService.UploadServiceListener{
    ...
    public void onDownloadFile(){
        ...
        request.path_folder_output  = SuperSafeApplication.getInstance().getSupersafePrivate();
    ...
    public void onUploadFile(final File file){
        ...
        Call<DriveResponse> request = SuperSafeApplication.serverAPI.uploadFileMutil(getString(R.string.url_drive_upload),mUser.access_token,metaPart,dataPart,Utils.getMimeType(file.getAbsolutePath()));
    ...
    public void onUploadFileInAppFolder(final File file){
        ...
        Call<DriveResponse> request = SuperSafeApplication.serverAPI.uploadFileMultipleInAppFolder(getString(R.string.url_drive_upload),mUser.access_token,metaPart,dataPart,Utils.getMimeType(file.getAbsolutePath()));
        request.enqueue(new Callback<DriveResponse>(){
            @Override
            public void onResponse(Call<DriveResponse> call, Response<DriveResponse> response) {
                Log.d(TAG,"response successful :"+ new Gson().toJson(response.body()));
