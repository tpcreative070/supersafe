package co.tpcreative.supersafe.ui.checksystemimport
...
class CheckSystemPresenter : Presenter<BaseView<*>?>() {
    ...
    fun getIntent(activity: Activity?) {
        try {
            ...
            Utils.Companion.Log(TAG, "≈ " + Gson().toJson(googleOauth))
        } catch (e: Exception) {
            e.printStackTrace()
        }
    }
    ...
    fun onCheckUser(email: String?, other_email: String?) {
        Utils.Companion.Log(TAG, "onCheckUser")
        ...
        subscriptions.add(SuperSafeApplication.Companion.serverAPI.onCheckUserId(UserRequest())
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .doOnSubscribe(Consumer<Disposable?> { __: Disposable? -> view.onStartLoading(EnumStatus.USER_ID_EXISTING) })
                .subscribe(Consumer<RootResponse?> { onResponse: RootResponse? ->
                    ...
                    if (throwable is HttpException) {
                        ...
                        Utils.Companion.Log(TAG, "Can not call check user" + throwable.message)
                    }
    ...
    fun onSignIn(request: SignInRequest?) {
        ...
        subscriptions.add(SuperSafeApplication.Companion.serverAPI.onSignIn(request)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .doOnSubscribe(Consumer<Disposable?> { __: Disposable? -> view.onStartLoading(EnumStatus.SIGN_IN) })
                .subscribe(Consumer<RootResponse?> { onResponse: RootResponse? ->
                    if (onResponse.error) {
                        ...
                        ServiceManager.Companion.getInstance().onInitConfigurationFile()
                    }
                    Utils.Companion.Log(TAG, "Body : " + Gson().toJson(onResponse))
                }, Consumer { throwable: Throwable? ->
                    if (throwable is HttpException) {
                        ...
                        Utils.Companion.Log(TAG, "Can not call sign in" + throwable.message)
                    }
    ...
    fun onVerifyCode(request: VerifyCodeRequest?) {
        Utils.Companion.Log(TAG, "onVerifyCode")
        val view: BaseView<*> = view() ?: return
        if (NetworkUtil.pingIpAddress(view.getContext())) {
            return
        }
        ...
        subscriptions.add(SuperSafeApplication.Companion.serverAPI.onVerifyCode(request)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .doOnSubscribe(Consumer<Disposable?> { __: Disposable? -> view.onStartLoading(EnumStatus.VERIFY_CODE) })
                .subscribe(Consumer<RootResponse?> { onResponse: RootResponse? ->
                    if (onResponse.error) {
                    ...
                    Utils.Companion.Log(TAG, "Body verify code : " + Gson().toJson(onResponse))
                }, Consumer { throwable: Throwable? ->
                    if (throwable is HttpException) {
                        val bodys: ResponseBody? = (throwable as HttpException?).response().errorBody()
                        val code = (throwable as HttpException?).response().code()
                        ...
                        Utils.Companion.Log(TAG, "Can not call verify code" + throwable.message)
                    }
    ...
    fun onChangeEmail(request: VerifyCodeRequest?) {
        ...
        subscriptions.add(SuperSafeApplication.Companion.serverAPI.onUpdateUser(ChangeUserIdRequest(request))
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .doOnSubscribe(Consumer<Disposable?> { __: Disposable? -> view.onStartLoading(EnumStatus.CHANGE_EMAIL) })
                .subscribe(Consumer<RootResponse?> { onResponse: RootResponse? ->
                        ...
                        if (onResponse != null) {
                            ...
                            if (mData.author != null) {
                                if (mUser != null) {
                                    ...
                                    Utils.Companion.Log(TAG, "User is null")
                                }
                                mUser = Utils.Companion.getUserInfo()
    ...
    fun onResendCode(request: VerifyCodeRequest?) {
        ...
        subscriptions.add(SuperSafeApplication.Companion.serverAPI.onResendCode(RequestCodeRequest(request.user_id, Utils.Companion.getAccessToken(), SuperSafeApplication.Companion.getInstance().getDeviceId()))
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .doOnSubscribe(Consumer<Disposable?> { __: Disposable? -> view.onStartLoading(EnumStatus.RESEND_CODE) })
                .subscribe(Consumer<RootResponse?> { onResponse: RootResponse? ->
                    if (onResponse.error) {
