package co.tpcreative.supersafe.ui.premiumimport
...
class PremiumActivity : BaseActivity(), BaseView<Any?>, IBillingHandler {
    ...
    protected override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (fragment == null) {
            val mFactory: FragmentFactory = getSupportFragmentManager().getFragmentFactory()
            fragment = mFactory.instantiate(ClassLoader.getSystemClassLoader(), co.tpcreative.supersafe.ui.premium.PremiumActivity.SettingsFragment::class.java.name)
        }
    ...
    fun onUpdatedView() {
        val isPremium: Boolean = Utils.Companion.isPremium()
        if (isPremium) {
            tvTitle.setText(getText(R.string.you_are_in_premium_features))
            tvPremiumLeft.setVisibility(View.GONE)
            llTwo.setVisibility(View.VISIBLE)
        } else {
            val sourceString: String = Utils.Companion.getFontString(R.string.upgrade_premium_to_use_full_features, getString(R.string.premium_uppercase))
    ...
    fun onClickedMonths(view: View?) {
        if (BillingProcessor.isIabServiceAvailable(this)) {
            Utils.Companion.Log(TAG, "purchase new")
            if (bp.isSubscribed(getString(R.string.six_months))) {
                Utils.Companion.Log(TAG, "Already charged")
    ...
    fun onStartInAppPurchase() {
        bp = BillingProcessor(this, Utils.Companion.GOOGLE_CONSOLE_KEY, this)
        bp.initialize()
    }
    ...
    override fun onSuccessful(message: String?, status: EnumStatus?) {
        if (status == EnumStatus.CHECKOUT) {
            Utils.Companion.Log(TAG, message + "-" + status.name)
            onUpdatedView()
        }
    }
    ...
    override fun onProductPurchased(productId: String?, details: TransactionDetails?) {
        Utils.Companion.Log(TAG, "Checkout detail " + Gson().toJson(details))
        if (details != null) {
    ...
    override fun onPurchaseHistoryRestored() {
        Utils.Companion.Log(TAG, "onPurchaseHistoryRestored")
    }

    override fun onBillingError(errorCode: Int, error: Throwable?) {
        Utils.Companion.Log(TAG, "onBillingError $error")
    }

    override fun onBillingInitialized() {
        Utils.Companion.Log(TAG, "onBillingInitialized")
        ...
        if (mTransaction != null) {
            Utils.Companion.Log(TAG, "Result of 6 months " + Gson().toJson(mTransaction))
        }
    }

    fun onCheckout(data: PurchaseData?, purchase: EnumPurchase?) {
        var mCheckout: CheckoutItems? = Utils.Companion.getCheckoutItems()
        Utils.Companion.Log(TAG, "Call checkout....")
        when (purchase) {
            EnumPurchase.LIFETIME -> {
                if (mCheckout != null) {
                    mCheckout.isPurchasedLifeTime = Utils.Companion.isRealCheckedOut(data.orderId)
                } else {
                    mCheckout = CheckoutItems()
                    mCheckout.isPurchasedLifeTime = Utils.Companion.isRealCheckedOut(data.orderId)
                }
                Utils.Companion.setCheckoutItems(mCheckout)
            }
            EnumPurchase.SIX_MONTHS -> {
                if (mCheckout != null) {
                    if (Utils.Companion.isRealCheckedOut(data.orderId)) {
            ...
            else -> Utils.Companion.setCheckoutItems(CheckoutItems())
        }
