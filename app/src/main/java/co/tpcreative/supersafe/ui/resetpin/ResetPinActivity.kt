package co.tpcreative.supersafe.ui.resetpinimport
...
class ResetPinActivity : BaseVerifyPinActivity(), BaseView<Any?>, OnEditorActionListener {
    ...
    protected override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (presenter.mUser != null) {
            ...
            if (email != null) {
                val result: String = Utils.Companion.getFontString(R.string.request_an_access_code, email)
                tvStep1.setText(Html.fromHtml(result))
                tvStep1.setText(Html.fromHtml(result))
                val support: String = Utils.Companion.getFontString(R.string.send_an_email_to, SecurityUtil.MAIL)
    ...
    fun onMessageEvent(event: EnumStatus?) {
        when (event) {
            ...
            EnumStatus.WAITING_LEFT -> {
                runOnUiThread(Runnable { edtCode.setHint(kotlin.String.format(getString(R.string.waiting_left), SingletonResetPin.Companion.getInstance().waitingLeft.toString() + "")) })
            }
    ...
    fun setProgressValue() {
        ...
        val b = CircularProgressDrawable.Builder(this)
                ...
                .sweepSpeed(2f)
                .rotationSpeed(2f)
                .strokeWidth(Utils.Companion.dpToPx(3).toFloat())
        ...
        Utils.Companion.Log(TAG, "Action here set progress")
    }

    private val mTextWatcher: TextWatcher? = object : TextWatcher {
        override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
            val value = s.toString().trim { it <= ' ' }
            isNext = if (Utils.Companion.isValid(value)) {
    ...
    override fun onEditorAction(textView: TextView?, actionId: Int, keyEvent: KeyEvent?): Boolean {
        if (actionId == EditorInfo.IME_ACTION_DONE) {
            if (!SuperSafeReceiver.Companion.isConnected()) {
                Utils.Companion.showDialog(this, getString(R.string.internet))
                return false
            }
            if (isNext) {
                onVerifyCode()
    ...
    override fun onStopLoading(status: EnumStatus?) {
        Utils.Companion.Log(TAG, "Stop progressing")
    ...
    override fun onError(message: String?, status: EnumStatus?) {
        ...
        when (status) {
            EnumStatus.REQUEST_CODE -> {
                ...
                Utils.Companion.showGotItSnackbar(tvStep1, R.string.request_code_occurred_error)
            }
            EnumStatus.VERIFY -> {
                Utils.Companion.showGotItSnackbar(tvStep1, R.string.verify_occurred_error)
            }
        }
    }
    ...
    fun onShowDialogWaitingCode() {
        val themeApp: ThemeApp = ThemeApp.Companion.getInstance().getThemeInfo()
        val mUser: User = Utils.Companion.getUserInfo()
        Utils.Companion.Log(TAG, "Preparing " + Gson().toJson(mUser))
        MaterialStyledDialog.Builder(this)
                ...
                .onPositive(object : SingleButtonCallback {
                    override fun onClick(dialog: MaterialDialog, which: DialogAction) {
                        ...
                        val mUser: User = Utils.Companion.getUserInfo()
                        Utils.Companion.Log(TAG, "Pressed " + Gson().toJson(mUser))
