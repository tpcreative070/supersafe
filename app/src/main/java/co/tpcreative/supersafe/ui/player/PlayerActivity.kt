package co.tpcreative.supersafe.ui.playerimport
...
class PlayerActivity : BasePlayerActivity(), BaseView<Any?>, PlayerAdapter.ItemSelectedListener {
    ...
    override fun onClickGalleryItem(position: Int) {
        Utils.Companion.Log(TAG, "Position :$position")
    ...
    fun playVideo() {
        ...
        try {
            ...
            val concatenatedSource = ConcatenatingMediaSource(
                    *presenter.mListSource.toTypedArray())
            val haveStartPosition = lastWindowIndex != C.INDEX_UNSET
            if (haveStartPosition) {
                player.seekTo(lastWindowIndex, seekTo)
                Utils.Companion.Log(TAG, "Return value $lastWindowIndex - $seekTo")
            } else {
                Utils.Companion.Log(TAG, "No value $lastWindowIndex - $seekTo")
            }
            ...
            player.addListener(object : Player.EventListener {
                override fun onTimelineChanged(timeline: Timeline?, manifest: Any?, reason: Int) {
                    Utils.Companion.Log(TAG, "1")
                }

                override fun onTracksChanged(trackGroups: TrackGroupArray?, trackSelections: TrackSelectionArray?) {
                    Utils.Companion.Log(TAG, "2")
                }

                override fun onLoadingChanged(isLoading: Boolean) {
                    Utils.Companion.Log(TAG, "3")
                }

                override fun onPlayerStateChanged(playWhenReady: Boolean, playbackState: Int) {
                    Utils.Companion.Log(TAG, "4 $playbackState")
                }

                override fun onRepeatModeChanged(repeatMode: Int) {
                    Utils.Companion.Log(TAG, "5")
                }

                override fun onShuffleModeEnabledChanged(shuffleModeEnabled: Boolean) {
                    Utils.Companion.Log(TAG, "6")
                }

                override fun onPlayerError(error: ExoPlaybackException?) {
                    Utils.Companion.Log(TAG, "7")
                }
                ...
                override fun onPositionDiscontinuity(reason: Int) {
                    ...
                    Utils.Companion.Log(TAG, "position ???????$lastWindowIndex")
                }

                override fun onPlaybackParametersChanged(playbackParameters: PlaybackParameters?) {
                    Utils.Companion.Log(TAG, "9")
                }

                override fun onSeekProcessed() {
                    Utils.Companion.Log(TAG, "10")
                }
            })
        } catch (e: Exception) {
            e.printStackTrace()
    ...
    fun onClickedBack(view: View?) {
        val isLandscape: Boolean = Utils.Companion.isLandscape(this)
        if (isLandscape) {
            setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT)
            isPortrait = true
            Utils.Companion.Log(TAG, "Request SCREEN_ORIENTATION_PORTRAIT")
        } else {
    ...
    override fun onSuccessful(message: String?, status: EnumStatus?) {
        when (status) {
            EnumStatus.PLAY -> {
                ...
                Utils.Companion.Log(TAG, mEncryptedFile.getAbsolutePath())
                if (mCipher == null) {
                    Utils.Companion.Log(TAG, " mcipher is null")
                    return
                }
    ...
    protected override fun onSaveInstanceState(outState: Bundle?) {
        ...
        Utils.Companion.Log(TAG, "Saved------------------------ " + seekPosition + " - " + player.getCurrentWindowIndex())
    }
    ...
    protected override fun onRestoreInstanceState(savedInstanceState: Bundle?) {
        ...
        Utils.Companion.Log(TAG, "Restore $seekTo")
    }
