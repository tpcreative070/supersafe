package co.tpcreative.supersafe.ui.cameraimport
...
class CameraActivity : BaseActivity(), OnRequestPermissionsResultCallback {
        ...
        when (v.id) {
            R.id.take_picture -> if (mCameraView != null) {
                if (GalleryCameraMediaManager.Companion.getInstance().isProgressing()) {
                    Utils.Companion.Log(TAG, "Progressing")
                    break
                }
            ...
            R.id.btnFlash -> if (mCameraView != null) {
                ...
                btnFlash.setImageResource(FLASH_ICONS.get(mCurrentFlash))
    ...
    fun onClickedFocus(view: View?) {
        if (mCameraView != null) {
            if (mCameraView.getGrid() == Grid.OFF) {
                btnAutoFocus.setColorFilter(SuperSafeApplication.Companion.getInstance().getResources().getColor(R.color.white), PorterDuff.Mode.SRC_IN)
                mCameraView.setGrid(Grid.DRAW_3X3)
            } else {
                btnAutoFocus.setColorFilter(SuperSafeApplication.Companion.getInstance().getResources().getColor(themeApp.getAccentColor()), PorterDuff.Mode.SRC_IN)
...
//        }
    ...
    override fun onBackPressed() {
        if (isReload) {
            ...
            Utils.Companion.Log(TAG, "onBackPressed")
        }
        super.onBackPressed()
    }
    ...
    private val mCallback: CameraListener? = object : CameraListener() {
        ...
        override fun onPictureTaken(result: PictureResult) {
            ...
            Toast.makeText(this@CameraActivity, R.string.picture_taken, Toast.LENGTH_SHORT).show()
            if (mainCategories == null) {
                Utils.Companion.Log(TAG, "Local id is null")
                Utils.Companion.onWriteLog("Main categories is null", EnumStatus.WRITE_FILE)
                return
            }
            isReload = true
            ServiceManager.Companion.getInstance().onSaveDataOnCamera(data, mainCategories)
            Utils.Companion.Log(TAG, "take picture")
        }
