package co.tpcreative.supersafe.ui.signupimport
...
class SignUpActivity : BaseActivityNoneSlide(), OnEditorActionListener, BaseView<User?> {
    ...
    override fun onEditorAction(textView: TextView?, actionId: Int, keyEvent: KeyEvent?): Boolean {
        if (actionId == EditorInfo.IME_ACTION_NEXT) {
            if (!SuperSafeReceiver.Companion.isConnected()) {
                Utils.Companion.showDialog(this@SignUpActivity, getString(R.string.internet))
                return false
            }
            if (isEmail && isName) {
    ...
    private val mTextWatcher: TextWatcher? = object : TextWatcher {
        override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
            if (getCurrentFocus() === edtName) {
                isName = if (Utils.Companion.isValid(s)) {
                    true
                } else {
                    false
                }
            ...
            if (getCurrentFocus() === edtEmail) {
                isEmail = if (Utils.Companion.isValidEmail(s)) {
                    true
                } else {
                    false
                }
    ...
    fun onSignUp() {
        ...
        Utils.Companion.hideSoftKeyboard(this)
        Utils.Companion.Log(TAG, "onFished")
    }
