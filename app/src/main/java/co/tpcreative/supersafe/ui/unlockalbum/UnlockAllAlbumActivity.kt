...
class UnlockAllAlbumActivity : BaseActivity(), BaseView<Any?>, OnEditorActionListener {
    ...
    fun onUnlockAlbums(view: View?) {
        Utils.Companion.Log(TAG, "Action")
        if (isNext) {
            btnUnlock.setText("")
            onStartLoading(EnumStatus.UNLOCK_ALBUMS)
            Utils.Companion.Log(TAG, "onUnlockAlbums")
            btnUnlock.setEnabled(false)
            onVerifyCode()
        }
    }
    ...
    override fun onError(message: String?, status: EnumStatus?) {
        ...
        when (status) {
            EnumStatus.REQUEST_CODE -> {
                ...
                btnUnlock.setEnabled(true)
                Utils.Companion.showGotItSnackbar(tvStep1, message)
            }
    ...
    override fun onSuccessful(message: String?, status: EnumStatus?) {
        when (status) {
            ...
            EnumStatus.VERIFY -> {
                ...
                Utils.Companion.showGotItSnackbar(getCurrentFocus(), R.string.unlocked_successful, object : ServiceManagerSyncDataListener {
