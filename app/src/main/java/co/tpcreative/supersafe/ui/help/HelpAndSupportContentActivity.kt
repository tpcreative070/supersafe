...
class HelpAndSupportContentActivity : BaseActivity(), BaseView<Any?>, OnEditorActionListener {
    ...
    override fun onError(message: String?, status: EnumStatus?) {
        when (status) {
            EnumStatus.SEND_EMAIL -> {
                onStopProgressing()
                Utils.Companion.showGotItSnackbar(getCurrentFocus(), R.string.send_email_failed)
                edtSupport.setText("")
            }
        }
    }
    ...
    override fun onSuccessful(message: String?, status: EnumStatus?) {
        Utils.Companion.Log(TAG, Gson().toJson(presenter.content))
        when (status) {
            EnumStatus.RELOAD -> {
            ...
            EnumStatus.SEND_EMAIL -> {
                onStopProgressing()
                Utils.Companion.showInfoSnackbar(getCurrentFocus(), R.string.thank_you, true)
                edtSupport.setText("")
            }
        }
    }
    ...
    override fun onEditorAction(textView: TextView?, i: Int, keyEvent: KeyEvent?): Boolean {
        if (i == EditorInfo.IME_ACTION_DONE) {
            ...
            if (isNext) {
                ...
                val emailToken: EmailToken = EmailToken.Companion.getInstance().convertTextObject(mUser, content)
                presenter.onSendMail(emailToken, content)
                Utils.Companion.hideKeyboard(edtSupport)
                return true
            }
            return false
        }
