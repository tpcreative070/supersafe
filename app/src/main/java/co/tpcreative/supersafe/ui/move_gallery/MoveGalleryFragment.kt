package co.tpcreative.supersafe.ui.move_galleryimport
...
import co.tpcreative.supersafe.ui.move_gallery.MoveGalleryAdapter
...
class MoveGalleryFragment : Fragment(), MoveGalleryAdapter.ItemSelectedListener, MoveGalleryView {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Utils.Companion.Log(TAG, Gson().toJson(mConfig))
    ...
    fun onMessageEvent(event: EnumStatus?) {
        when (event) {
            EnumStatus.UPDATE_MOVE_NEW_ALBUM -> {
                if (mConfig != null) {
                    presenter.getData(mConfig.localCategoriesId, mConfig.isFakePIN)
                    Utils.Companion.Log(TAG, "Updated UI => Warning categories id is null")
                }
            }
        }
    }
    ...
    override fun onAttach(activity: Activity?) {
        ...
        mListener = try {
            Utils.Companion.Log(TAG, "Register Listener")
    ...
    fun openAlbum() {
        ...
        val screenHeight: Int = Utils.Companion.getScreenHeight(activity)
        ...
        if (mConfig.dialogMode >= Configuration.Companion.DIALOG_GRID) {
        ...
        if (mConfig.dialogHeight < 0) {
            mBehavior.setPeekHeight(if (mConfig.dialogHeight <= Configuration.Companion.DIALOG_HALF) screenHeight / 2 else screenHeight)
        } else {
            mBehavior.setPeekHeight(if (mConfig.dialogHeight >= screenHeight) screenHeight else mConfig.dialogHeight)
        }
        dialog.show()
    }
    ...
    private fun getGallerWidth(container: ViewGroup?): Int {
        return Utils.Companion.getScreenWidth(activity) - container.getPaddingLeft() - container.getPaddingRight()
    }
