package co.tpcreative.supersafe.ui.enablecloudimport
...
class EnableCloudActivity : BaseGoogleApi(), BaseView<Any?> {
    ...
    protected override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Utils.Companion.Log(TAG, "Enable cloud...........")
    }
    ...
    fun onGoogleDrive(view: View?) {
        ...
        if (cloud_id == null) {
            ...
            ServiceManager.Companion.getInstance().onPickUpExistingEmail(this, cloud_id)
        }
    }

    @OnClick(R.id.btnUserAnotherAccount)
    fun onUserAnotherAccount(view: View?) {
        Utils.Companion.Log(TAG, "user another account")
        btnUserAnotherAccount.setEnabled(false)
        btnLinkGoogleDrive.setEnabled(false)
    ...
    protected override fun onDriveClientReady() {
        Utils.Companion.Log(TAG, "Google drive ready")
        val request = UserCloudRequest(presenter.mUser.email, presenter.mUser.cloud_id, SuperSafeApplication.Companion.getInstance().getDeviceId())
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        when (requestCode) {
            ...
            Navigator.REQUEST_CODE_EMAIL -> if (resultCode == Activity.RESULT_OK) {
                ...
                Utils.Companion.Log(TAG, "accountName : $accountName")
                val cloud_id: String = presenter.mUser.cloud_id
                if (cloud_id == null) {
                    presenter.mUser.cloud_id = accountName
                    Utils.Companion.Log(TAG, "Call Sign out")
        ...
        MaterialStyledDialog.Builder(this)
                ...
                .onPositive(object : SingleButtonCallback {
                    override fun onClick(dialog: MaterialDialog, which: DialogAction) {
                        ...
                        val cloud_id: String = presenter.mUser.cloud_id
                        if (cloud_id == null) {
                            ServiceManager.Companion.getInstance().onPickUpNewEmailNoTitle(this@EnableCloudActivity, presenter.mUser.email)
                        } else {
                            ServiceManager.Companion.getInstance().onPickUpNewEmailNoTitle(this@EnableCloudActivity, cloud_id)
                        }
                    }
                })
                .onNegative(object : SingleButtonCallback {
    ...
    protected override fun onDriveSuccessful() {
        Utils.Companion.Log(TAG, "onDriveSuccessful")
    }
    ...
    override fun onError(message: String?, status: EnumStatus?) {
        Utils.Companion.Log(TAG, "" + message)
        when (status) {
            EnumStatus.CREATE -> {
                onStopProgressDialog()
            }
    ...
    override fun onSuccessful(message: String?, status: EnumStatus?) {
        when (status) {
            EnumStatus.CREATE -> {
                ...
                Utils.Companion.Log(TAG, "Finsh enable cloud.........................")
