...
class PrivateFragment : BaseFragment(), BaseView<Any?>, PrivateAdapter.ItemSelectedListener, SingletonPrivateFragment.SingletonPrivateFragmentListener {
    ...
    fun initRecycleView(layoutInflater: LayoutInflater?) {
        adapter = PrivateAdapter(layoutInflater, getContext(), this)
    ...
    override fun onClickItem(position: Int) {
        if (isClicked) {
            Utils.Companion.Log(TAG, "Deny onClick $position")
            return
        }
        Log.d(TAG, "Position :$position")
        try {
            val value: String = Utils.Companion.getHexCode(getString(R.string.key_trash))
    ...
    override fun onDeleteAlbum(position: Int) {
        Utils.Companion.Log(TAG, "Delete album")
        DialogManager.Companion.getInstance().onStartDialog(getContext(), R.string.confirm, R.string.are_you_sure_you_want_to_move_this_album_to_trash, object : DialogListener {
    ...
    override fun onEmptyTrash(position: Int) {
        try {
            DialogManager.Companion.getInstance().onStartDialog(getContext(), R.string.delete_all, R.string.are_you_sure_you_want_to_empty_trash, object : DialogListener {
                override fun onClickButton() {
                ...
                try {
                    getActivity().runOnUiThread(Runnable {
                        if (adapter != null) {
                            ...
                            Utils.Companion.Log(TAG, "Reload")
                        }
                    })
                } catch (e: Exception) {
                    e.message
