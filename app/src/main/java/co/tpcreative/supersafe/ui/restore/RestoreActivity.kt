package co.tpcreative.supersafe.ui.restoreimport
...
class RestoreActivity : BaseActivity(), OnEditorActionListener, BaseView<Any?> {
        ...
        if (actionId == EditorInfo.IME_ACTION_DONE) {
            ...
            if (isNext) {
                ...
                if (pin == edtPreviousPIN.getText().toString()) {
                    Utils.Companion.hideKeyboard(getCurrentFocus())
                    onStartProgressing()
                    Utils.Companion.onObserveData(2000, Listener { onRestore() })
                } else {
    ...
    private val mTextWatcher: TextWatcher? = object : TextWatcher {
        override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
            ...
            if (Utils.Companion.isValid(value)) {
