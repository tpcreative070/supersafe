...
class SettingsActivity : BaseActivity() {
    ...
    protected override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (fragment == null) {
            fragment = Fragment.instantiate(this, SettingsFragment::class.java.name)
        }
    ...
    protected override fun setStatusBarColored(context: AppCompatActivity?, colorPrimary: Int, colorPrimaryDark: Int) {
        context.getSupportActionBar().setBackgroundDrawable(ColorDrawable(getResources()
        ...
        when (requestCode) {
            Navigator.THEME_SETTINGS -> {
                if (resultCode == Activity.RESULT_OK) {
                    Utils.Companion.Log(TAG, "recreate........................")
                }
            }
            Navigator.ENABLE_CLOUD -> {
                Utils.Companion.Log(TAG, "onResultResponse :$resultCode")
                if (resultCode == Activity.RESULT_OK) {
            ...
            return Preference.OnPreferenceClickListener { preference ->
                if (preference is Preference) {
                    if (preference.key == getString(R.string.key_account)) {
                        ...
                        Utils.Companion.Log(TAG, "Log here")
