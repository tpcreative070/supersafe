...
class VerifyAccountActivity : BaseActivity(), OnEditorActionListener, BaseView<Any?> {
    ...
    override fun onEditorAction(textView: TextView?, i: Int, keyEvent: KeyEvent?): Boolean {
        if (i == EditorInfo.IME_ACTION_DONE) {
            ...
            if (isNext) {
                Utils.Companion.Log(TAG, "Next")
                if (getCurrentFocus() === edtCode) {
                    onVerifyCode()
                }
                if (getCurrentFocus() === edtEmail) {
                    Utils.Companion.hideSoftKeyboard(this)
                    onChangedEmail()
                }
                return true
            }
    ...
    private val mTextWatcher: TextWatcher? = object : TextWatcher {
        override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
            ...
            if (getCurrentFocus() === edtEmail) {
                isNext = if (Utils.Companion.isValidEmail(value)) {
    ...
    fun onClickedSendVerifyCode(view: View?) {
        Utils.Companion.Log(TAG, "Verify code")
        SingletonManagerProcessing.Companion.getInstance().onStartProgressing(this, R.string.progressing)
        presenter.onCheckUser(presenter.mUser.email, presenter.mUser.other_email)
    }

    @OnClick(R.id.btnCancel)
    fun onClickedCancel(view: View?) {
        Utils.Companion.Log(TAG, "onCancel")
        Utils.Companion.hideSoftKeyboard(this)
        onShowView(llAction)
    }
    ...
    fun onClickedSave() {
        Utils.Companion.Log(TAG, "onSave")
        if (isNext) {
            onChangedEmail()
            /*Do something here*/
        }
    ...
    fun onChangedEmail() {
        ...
        Utils.Companion.hideSoftKeyboard(this)
        Utils.Companion.hideKeyboard(edtEmail)
    }
    ...
    fun onClickedResend() {
        try {
            ...
            Utils.Companion.Log(TAG, "onResend")
        } catch (e: Exception) {
        }
    }
        ...
        MaterialStyledDialog.Builder(this)
                ...
                .setCheckBox(true, R.string.enable_cloud, object : CompoundButton.OnCheckedChangeListener {
                    override fun onCheckedChanged(compoundButton: CompoundButton?, b: Boolean) {
                        Utils.Companion.Log(TAG, "checked :$b")
                        isSync = b
                    }
                })
                .onPositive(object : SingleButtonCallback {
                    override fun onClick(dialog: MaterialDialog, which: DialogAction) {
                        ...
                        ServiceManager.Companion.getInstance().onPickUpNewEmail(this@VerifyAccountActivity)
                    }
                })
                .show()
    }
    ...
    override fun onSuccessful(message: String?, status: EnumStatus?) {
        when (status) {
            ...
            EnumStatus.SEND_EMAIL -> {
                ...
                Utils.Companion.showGotItSnackbar(tvEmail, R.string.we_sent_access_code_to_your_email)
            }
            EnumStatus.VERIFY_CODE -> {
                edtCode.setText("")
