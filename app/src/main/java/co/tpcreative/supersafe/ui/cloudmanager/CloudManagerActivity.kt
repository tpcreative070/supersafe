package co.tpcreative.supersafe.ui.cloudmanagerimport
...
class CloudManagerActivity : BaseGoogleApi(), CompoundButton.OnCheckedChangeListener, BaseView<Long?> {
    ...
    fun onShowUI() {
        ...
        if (mUser != null) {
            ...
            try {
                val superSafeSpace: String = ConvertUtils.Companion.byte2FitMemorySize(driveAbout.inAppUsed)
            ...
            try {
                ...
                if (storageQuota != null) {
                    val superSafeSpace: String = ConvertUtils.Companion.byte2FitMemorySize(storageQuota.usage)
            ...
            try {
                ...
                if (storageQuota != null) {
                    ...
                    val superSafeSpace: String = ConvertUtils.Companion.byte2FitMemorySize(result)
    ...
    override fun onError(message: String?, status: EnumStatus?) {
        when (status) {
            EnumStatus.REQUEST_ACCESS_TOKEN -> {
                Utils.Companion.Log(TAG, "Error response $message")
                getAccessToken()
            }
            else -> {
                Utils.Companion.Log(TAG, "Error response $message")
    ...
    override fun onSuccessful(message: String?, status: EnumStatus?) {
        when (status) {
            ...
            EnumStatus.SAVER -> {
                tvDeviceSaving.setText(ConvertUtils.Companion.byte2FitMemorySize(presenter.sizeSaverFiles))
            }
    ...
    override fun onSuccessful(message: String?, status: EnumStatus?, list: MutableList<*>?) {
        Utils.Companion.Log(TAG, "Successful response $message")
    }
    ...
    override fun onCheckedChanged(compoundButton: CompoundButton?, b: Boolean) {
        Utils.Companion.Log(TAG, "onCheckedChanged...............!!!")
    ...
    protected override fun onDestroy() {
        ...
        if (isDownload) {
            ...
            ServiceManager.Companion.getInstance().onPreparingSyncData()
            Utils.Companion.Log(TAG, "Re-Download file")
        }
        if (isSpaceSaver) {
    ...
    protected override fun onDriveError() {
        Utils.Companion.Log(TAG, "onDriveError")
    }

    protected override fun onDriveSignOut() {
        Utils.Companion.Log(TAG, "onDriveSignOut")
    }

    protected override fun onDriveRevokeAccess() {
        Utils.Companion.Log(TAG, "onDriveRevokeAccess")
    }
    ...
    fun onShowDialog() {
        ...
        space_required.setText(kotlin.String.format(getString(R.string.space_required), ConvertUtils.Companion.byte2FitMemorySize(presenter.sizeFile)))
        val builder: com.afollestad.materialdialogs.MaterialDialog.Builder = com.afollestad.materialdialogs.MaterialDialog.Builder(this)
                ...
                .onNegative(object : SingleButtonCallback {
                    override fun onClick(dialog: com.afollestad.materialdialogs.MaterialDialog, which: DialogAction) {
                        Utils.Companion.Log(TAG, "negative")
