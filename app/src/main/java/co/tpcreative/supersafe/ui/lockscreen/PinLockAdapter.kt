...
class PinLockAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder?>() {
    ...
    private fun configureDeleteButtonHolder(holder: VerifyViewHolder?) {
        if (holder != null) {
            if (mCustomizationOptionsBundle.isShowVerifyButton() && mPinLength > 0) {
                ...
                Utils.Companion.Log(TAG, "onVerify changed color")
    ...
    private fun getAdjustKeyValues(keyValues: IntArray?): IntArray? {
        ...
        for (i in keyValues.indices) {
            if (i < 9) {
                adjustedKeyValues[i] = keyValues.get(i)
            } else {
                adjustedKeyValues[i] = -1
                adjustedKeyValues[i + 1] = keyValues.get(i)
            }
        }
    ...
    interface OnNumberClickListener {
        open fun onNumberClicked(keyValue: Int)
    }

    interface OnVerifyClickListener {
        open fun onVerifyClicked()
    }
        ...
        fun onVerifyButton() {
            if (mCustomizationOptionsBundle.isShowVerifyButton() && mPinLength > 0) {
                if (mOnVerifyClickListener != null) {
                    ...
                    Utils.Companion.Log(TAG, "Verified button")
                } else {
                    Utils.Companion.Log(TAG, "mOnVerifyClickListener Null")
                }
            } else {
                Utils.Companion.Log(TAG, "Pin length Null")
            }
        }
