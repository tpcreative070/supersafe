...
class CloudManagerPresenter : Presenter<BaseView<Long?>?>() {
    ...
    fun onDisableSaverSpace(enumStatus: EnumStatus?) {
        ...
        Utils.Companion.Log(TAG, Gson().toJson(mList))
    }
            ...
            for (i in mList.indices) {
                ...
                when (formatType) {
                    EnumFormatType.IMAGE -> {
                        ...
                        Utils.Companion.Log(TAG, "Continue updating...")
                    }
                }
            }
        } else {
            Utils.Companion.Log(TAG, "Already saver files")
        }
        onGetSaveData()
    }

    fun onGetDriveAbout() {
        Utils.Companion.Log(TAG, "onGetDriveAbout" + "--" + EnumStatus.GET_DRIVE_ABOUT.name)
    ...
    private fun onGetList() {
        Utils.Companion.Log(TAG, "onGetList" + "--" + EnumStatus.GET_LIST_FILES_IN_APP.name)
        val view: BaseView<*> = view()
        if (NetworkUtil.pingIpAddress(view.getContext())) {
        ...
        ServiceManager.Companion.getInstance().getMyService().onGetListFileInApp(object : BaseView<Int?> {
            override fun onError(message: String?, status: EnumStatus?) {
                view.onError(message, status)
                Utils.Companion.Log(TAG, "error")
            }

            override fun onSuccessful(message: String?) {
                Utils.Companion.Log(TAG, "onSuccessful ??")
            }

            override fun onSuccessful(message: String?, status: EnumStatus?) {
                Utils.Companion.Log(TAG, "onSuccessful !! " + status.name)
            }
            ...
            override fun onSuccessful(message: String?, status: EnumStatus?, `object`: Int?) {
                if (`object` == 0) {
                    ...
                    try {
                        ...
                        Utils.Companion.Log(TAG, "onSuccessful 5")
                    } catch (e: Exception) {
                        Utils.Companion.Log(TAG, "onSuccessful 6")
                        e.printStackTrace()
                    }
                }
            }
