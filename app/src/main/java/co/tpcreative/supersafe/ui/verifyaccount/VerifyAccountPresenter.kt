package co.tpcreative.supersafe.ui.verifyaccountimport
...
class VerifyAccountPresenter : Presenter<BaseView<*>?>() {
    var mUser: User?
    ...
    fun onCheckUser(email: String?, other_email: String?) {
        Utils.Companion.Log(TAG, "info onCheckUser :$email")
        val view: BaseView<*> = view() ?: return
        if (NetworkUtil.pingIpAddress(view.getContext())) {
            return
        }
        ...
        subscriptions.add(SuperSafeApplication.Companion.serverAPI.onCheckUserId(UserRequest())
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .doOnSubscribe(Consumer<Disposable?> { __: Disposable? -> view.onStartLoading(EnumStatus.CHECK) })
                .subscribe(Consumer<RootResponse?> { onResponse: RootResponse? ->
                    Utils.Companion.Log(TAG, "Body : " + Gson().toJson(onResponse))
                    if (onResponse.error) {
    ...
    fun onSignIn(request: SignInRequest?) {
        ...
        Utils.Companion.Log(TAG, Gson().toJson(mUser))
        subscriptions.add(SuperSafeApplication.Companion.serverAPI.onSignIn(request)
                ...
                .subscribe(Consumer<RootResponse?> { onResponse: RootResponse? ->
                    if (onResponse.error) {
                        ...
                        val mData: DataResponse = onResponse.data
                        mUser = mData.user
                        Utils.Companion.setUserPreShare(mData.user)
                        val emailToken: EmailToken = EmailToken.Companion.getInstance().convertObject(mUser, EnumStatus.SIGN_IN)
                        onSendMail(emailToken)
    ...
    fun onRefreshEmailToken(request: EmailToken?) {
        Utils.Companion.Log(TAG, "onRefreshEmailToken....." + Gson().toJson(request))
        val view: BaseView<*> = view() ?: return
        if (NetworkUtil.pingIpAddress(SuperSafeApplication.Companion.getInstance())) {
            return
        }
